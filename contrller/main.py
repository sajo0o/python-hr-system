...
class MyWidget(QWidget,Ui_Form):
    ...
    def filter_emps_by_name(self):
        ...
        if query != "":
            for i,e in enumerate(self.emps):
                if not e.emp_name.startswith(query):
                    self.tb_emps.hideRow(i)
    ...
    def show_add_dept_dialog(self):
        ...
        if choice == 1:
            ...
            self.sb_depts.addItem(d.dept_name)
            self.depts.append(d)
            d.save_to_db()

    def delete_dept(self):
        try:
            idx = self.sb_depts.currentIndex()
            if idx != 0:
                self.sb_depts.removeItem(idx)
